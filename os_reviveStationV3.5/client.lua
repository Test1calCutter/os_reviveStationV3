                    ...
                    if distance <= circleRadius and IsControlJustReleased(0, 38) then
                        if useBilling then
                            if okokBilling then
                                ...
                                ExecuteCommand("restart esx_billing") -- Restarting esx_billing to allow user pay bill
                                TriggerEvent('esx_ambulancejob:revive')
                            end
                        else
                            TriggerEvent('esx_ambulancejob:revive')
                    ...
                    if distance <= circleRadius and IsControlJustReleased(0, 38) then
                        if useBilling then
                            if okokBilling then
                                ...
                                TriggerServerEvent('os_billing:sendBill', GetPlayerServerId(PlayerId()), 'society_'..job, _U('billing_reason'), revivePrice)
                                ExecuteCommand("restart esx_billing") -- Restarting esx_billing to allow user pay bill
                                TriggerEvent('esx_ambulancejob:revive')
                            end
                        else
